...
type Droplet = Vec<(i64, i64, i64)>;

fn do_the_thing(droplet: Droplet) -> usize {
    let mut counter = 0;
    for drop in &droplet {
        ...
        for adjustment in adjustments {
            match droplet.iter().find(|other_drop| { other_drop.0==drop.0+adjustment.0 
                                                                           && other_drop.1==drop.1+adjustment.1 
                                                                           && other_drop.2==drop.2+adjustment.2}) {
                Some(_) => {},
                None => { counter += 1; }
            }
        }
...
fn parse_input(input: &str) -> Droplet {
    let mut droplet: Droplet = vec![];
    for line in input.lines() {
        let mut x = 0;
        let mut y = 0;
        let mut z = 0;
        match sscanf!(line, "{},{},{}", x, y, z) {
            Ok(_) => { droplet.push((x,y,z)); }
            _ => { panic!(); }
        }
    }
...
fn test_parse_sample_input() {
    ...
    assert_eq!((2,2,2), result[0]);
    assert_eq!((1,2,2), result[1]);
    assert_eq!((2,3,5), result[12]);
}
