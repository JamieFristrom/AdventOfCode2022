fn main() {
    println!("Hello, world!");

    let map = parse_map(get_puzzle_map_input());
    let path = parse_path(get_puzzle_path_input());
    let faces = vec![
        ((50, 0), [(1, 2), (2, 3), (3, 2), (5, 2)]),
        ((100, 0), [(4, 0), (2, 0), (0, 0), (5, 1)]),
        ((50, 50), [(1, 1), (4, 3), (3, 3), (0, 1)]),
        ((0, 100), [(4, 2), (5, 3), (0, 2), (2, 2)]),
        ((50, 100), [(1, 0), (5, 0), (3, 0), (2, 1)]),
        ((0, 150), [(4, 1), (1, 3), (0, 3), (3, 1)]),
    ];
    let cube_map = cubify_map(&map, 50, faces);
    let (position, facing) = navigate_cube_map(&cube_map, &path);
    println!("Position {:?}, facing {facing}", position);
    println!(
        "Answer {}",
        (position.1 + 1) * 1000 + 4 * (position.0 + 1) + facing as i64
    );
    // The final password is the sum of 1000 times the row, 4 times the column, and the facing.
}

#[derive(Debug, PartialEq)]
enum Move {
    Step(i64),
    Right,
    Left,
}

type Map = Vec<Vec<i8>>;

#[derive(Debug, PartialEq)]
struct Face {
    top_left: (usize, usize),
    bottom_right: (usize, usize),
    connections: [(usize, usize); 4],
}

type CubeMap = (Map, Vec<Face>);

const ADJUSTMENTS: [(i64, i64); 4] = [(1, 0), (0, 1), (-1, 0), (0, -1)];

fn navigate_map(map: &Vec<Vec<i8>>, path: &Vec<Move>) -> ((i64, i64), usize) {
    // You begin the path in the leftmost open tile of the top row of tiles.
    let mut position = (map[0].iter().position(|x| *x == 0).unwrap() as i64, 0i64);
    // Initially, you are facing to the right
    let mut facing = 0;

    let mut counter = 0;
    for movement in path {
        counter += 1;
        if counter % 10 == 0 {
            println!("{} steps of {}", counter, path.len());
        }
        if counter == 785 {
            println!("danger");
        }
        match movement {
            Move::Step(distance) => {
                position = step_forward(map, position, facing, *distance);
            }
            Move::Left => facing = if facing == 0 { 3 } else { facing - 1 },
            Move::Right => {
                facing += 1;
                if facing > 3 {
                    facing = 0;
                }
            }
        }
    }

    (position, facing)
}

// theoretically genericable but whateve
fn navigate_cube_map(cube_map: &CubeMap, path: &Vec<Move>) -> ((i64, i64), usize) {
    // You begin the path in the leftmost open tile of the top row of tiles.
    let mut position = (
        cube_map.0[0].iter().position(|x| *x == 0).unwrap() as i64,
        0i64,
    );
    // Initially, you are facing to the right
    let mut facing = 0;

    let mut counter = 0;
    for movement in path {
        counter += 1;
        if counter % 10 == 0 {
            println!("{} steps of {}", counter, path.len());
        }
        if counter == 785 {
            println!("danger");
        }
        match movement {
            Move::Step(distance) => {
                (position, facing) = cube_step_forward(&cube_map, position, facing, *distance);
                println!("New position {:?}, facing {facing}", position);
            }
            Move::Left => facing = if facing == 0 { 3 } else { facing - 1 },
            Move::Right => {
                facing += 1;
                if facing > 3 {
                    facing = 0;
                }
            }
        }
    }

    (position, facing)
}

#[test]
fn test_navigate_sample_map() {
    let map = parse_map(get_sample_map_input());
    let path = parse_path(get_sample_path_input());
    let (position, facing) = navigate_map(&map, &path);
    assert_eq!((7, 5), position);
    assert_eq!(0, facing);
    assert_eq!(6032, 1000 * (position.1 + 1) + 4 * (position.0 + 1) + 0);
}

#[test]
fn test_navigate_sample_cube_map() {
    let faces = vec![
        ((8, 0), [(5, 0), (3, 3), (2, 3), (1, 3)]),
        ((0, 4), [(2, 2), (4, 1), (5, 1), (0, 3)]),
        ((4, 4), [(3, 2), (4, 2), (1, 0), (0, 2)]),
        ((8, 4), [(5, 3), (4, 3), (2, 0), (0, 1)]),
        ((8, 8), [(5, 2), (1, 1), (2, 1), (3, 1)]),
        ((12, 8), [(0, 0), (1, 2), (4, 0), (3, 0)]),
    ];
    let map = cubify_map(&parse_map(get_sample_map_input()), 4, faces);
    let path = parse_path(get_sample_path_input());
    let (position, facing) = navigate_cube_map(&map, &path);
    assert_eq!((6, 4), position);
    assert_eq!(3, facing);
    assert_eq!(5031, 1000 * (position.1 + 1) + 4 * (position.0 + 1) + facing as i64);
}

fn v2add(v0: &(i64, i64), v1: &(i64, i64)) -> (i64, i64) {
    (v0.0 + v1.0, v0.1 + v1.1)
}

// don't have the patience to learn generics
fn v2add_usize(v0: &(usize, usize), v1: &(usize, usize)) -> (usize, usize) {
    (v0.0 + v1.0, v0.1 + v1.1)
}

fn wrap_pos(map: &Vec<Vec<i8>>, pos: &(i64, i64)) -> (i64, i64) {
    let new_y = if pos.1 < 0 {
        map.len() as i64 + pos.1
    } else {
        if pos.1 >= map.len() as i64 {
            pos.1 - map.len() as i64
        } else {
            pos.1
        }
    };
    let new_x = {
        if pos.0 < 0 {
            map[0].len() as i64 + pos.0
        } else {
            if pos.0 >= map[0].len() as i64 {
                pos.0 - map[0].len() as i64
            } else {
                pos.0
            }
        }
    };

    (new_x, new_y)
}

fn step_forward(map: &Vec<Vec<i8>>, pos: (i64, i64), facing: usize, num_steps: i64) -> (i64, i64) {
    let mut updated_pos = pos;
    for _ in 0..num_steps {
        let peek_pos = v2add(&updated_pos, &ADJUSTMENTS[facing]);
        let peek_pos = wrap_pos(map, &peek_pos);
        match map[peek_pos.1 as usize][peek_pos.0 as usize] {
            -1 => {
                let mut new_peek_pos = peek_pos;
                loop {
                    new_peek_pos = v2add(&new_peek_pos, &ADJUSTMENTS[facing]);
                    new_peek_pos = wrap_pos(map, &new_peek_pos);
                    match map[new_peek_pos.1 as usize][new_peek_pos.0 as usize] {
                        -1 => {} // keep looping
                        0 => {
                            // teleport here
                            updated_pos = new_peek_pos;
                            break;
                        }
                        1 => {
                            // wall blocks movement, we're done
                            return updated_pos;
                        }
                        _ => {
                            panic!();
                        }
                    }
                }
            }
            0 => {
                updated_pos = peek_pos;
            }
            1 => {
                return updated_pos;
            }
            _ => {
                panic!();
            }
        }
    }

    updated_pos
}

#[test]
fn test_sample_steps_forward() {
    let map = parse_map(get_sample_map_input());
    let pos = step_forward(&map, (8, 0), 1, 1);
    assert_eq!((8, 1), pos);
    let pos = step_forward(&map, (8, 1), 1, 1);
    assert_eq!((8, 1), pos);
    let pos = step_forward(&map, (8, 0), 2, 1);
    assert_eq!((8, 0), pos);
    let pos = step_forward(&map, (10, 0), 2, 2);
    assert_eq!((8, 0), pos);
    let pos = step_forward(&map, (10, 0), 2, 3);
    assert_eq!((8, 0), pos);
    let pos = step_forward(&map, (10, 1), 0, 1);
    assert_eq!((11, 1), pos);
    let pos = step_forward(&map, (10, 1), 0, 2);
    assert_eq!((8, 1), pos);
    let pos = step_forward(&map, (8, 6), 0, 5);
    assert_eq!((1, 6), pos);
}

fn get_face_idx(cube_map: &CubeMap, pos: &(i64, i64)) -> usize {
    cube_map
        .1
        .iter()
        .position(|face| {
            pos.0 >= face.top_left.0 as i64
                && pos.0 < face.bottom_right.0 as i64
                && pos.1 >= face.top_left.1 as i64
                && pos.1 < face.bottom_right.1 as i64
        })
        .unwrap()
}

fn get_face_offset(face_size: i64, facing: usize, new_edge: usize, pos: &(i64, i64)) -> (i64, i64) {
    // new_edge is opposite of facing the way I currently wrote it
    match facing {
        0 => match new_edge {
            0 => (face_size - 1, face_size - 1 - pos.1),
            1 => (pos.1, face_size - 1),
            2 => (0, pos.1),
            3 => (face_size - 1 - pos.1, 0),
            _ => panic!(),
        },
        1 => match new_edge {
            0 => (face_size - 1, pos.0),
            1 => (face_size - 1 - pos.0, face_size - 1),
            2 => (0, face_size - 1 - pos.0),
            3 => (pos.0, 0),
            _ => panic!(),
        },
        2 => match new_edge {
            0 => (face_size - 1, face_size - 1 - pos.1),
            1 => (face_size - 1 - pos.1, face_size - 1),
            2 => (0, face_size - 1 - pos.1),
            3 => (pos.1, 0),
            _ => panic!(),
        },
        3 => match new_edge {
            0 => (face_size - 1, face_size - 1 - pos.0),
            1 => (pos.0, face_size - 1),
            2 => (0, pos.0),
            3 => (face_size - 1 - pos.0, 0),
            _ => panic!(),
        },
        _ => panic!(),
    }
}

fn cube_step_forward(
    cube_map: &CubeMap,
    pos: (i64, i64),
    facing: usize,
    num_steps: i64,
) -> ((i64, i64), usize) {
    let mut facing = facing;
    let face_size = cube_map.1[0].bottom_right.0 - cube_map.1[0].top_left.0;
    let mut cur_face_idx = get_face_idx(cube_map, &pos);
    let mut updated_pos = pos;
    for _ in 0..num_steps {
        let peek_pos = v2add(&updated_pos, &ADJUSTMENTS[facing]);
        if peek_pos.0 < 0
            || peek_pos.1 < 0
            || peek_pos.0 >= cube_map.0[0].len() as i64
            || peek_pos.1 >= cube_map.0.len() as i64
            || cube_map.0[peek_pos.1 as usize][peek_pos.0 as usize] == -1
        {
            let cur_face = &cube_map.1[cur_face_idx];
            let (face_idx, edge_idx) = cur_face.connections[facing];
            let new_face = &cube_map.1[face_idx];
            let adj_pos = (
                peek_pos.0 - cur_face.top_left.0 as i64,
                peek_pos.1 - cur_face.top_left.1 as i64,
            );
            let face_offset = get_face_offset(face_size as i64, facing, edge_idx, &adj_pos);
            let new_peek_pos = v2add(
                &face_offset,
                &(new_face.top_left.0 as i64, new_face.top_left.1 as i64),
            );
            match cube_map.0[new_peek_pos.1 as usize][new_peek_pos.0 as usize] {
                -1 => {
                    panic!();
                }
                0 => {
                    // teleport here
                    updated_pos = new_peek_pos;
                    facing = (edge_idx + 2) % 4;
                }
                1 => {
                    // wall blocks movement, we're done
                    return (updated_pos, facing);
                }
                _ => {
                    panic!();
                }
            }
        } else if cube_map.0[peek_pos.1 as usize][peek_pos.0 as usize] == 0 {
            updated_pos = peek_pos;
        } else if cube_map.0[peek_pos.1 as usize][peek_pos.0 as usize] == 1 {
            return (updated_pos, facing);
        } else {
            panic!();
        }
    }

    (updated_pos, facing)
}

#[test]
fn test_cube_step_forward() {
    let faces = vec![
        ((8, 0), [(5, 0), (3, 3), (2, 3), (1, 3)]),
        ((0, 4), [(2, 2), (4, 1), (5, 1), (0, 3)]),
        ((4, 4), [(3, 2), (4, 2), (1, 0), (0, 2)]),
        ((8, 4), [(5, 3), (4, 3), (2, 0), (0, 1)]),
        ((8, 8), [(5, 2), (1, 1), (2, 1), (3, 1)]),
        ((12, 8), [(0, 0), (1, 2), (4, 0), (3, 0)]),
    ];
    let cube_map = cubify_map(&parse_map(&get_sample_map_input()), 4, faces);
    let (pos, facing) = cube_step_forward(&cube_map, (8, 0), 1, 1);
    assert_eq!((8, 1), pos);
    assert_eq!(1, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (8, 1), 1, 1);
    assert_eq!((8, 1), pos);
    assert_eq!(1, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (8, 0), 2, 1);
    assert_eq!((4, 4), pos);
    assert_eq!(1, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (10, 0), 2, 2);
    assert_eq!((8, 0), pos);
    assert_eq!(2, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (10, 0), 2, 3);
    assert_eq!((4, 4), pos);
    assert_eq!(1, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (10, 1), 0, 1);
    assert_eq!((11, 1), pos);
    let (pos, facing) = cube_step_forward(&cube_map, (10, 1), 0, 2);
    assert_eq!((15, 10), pos);
    assert_eq!(2, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (8, 6), 0, 5);
    assert_eq!((13, 8), pos);
    assert_eq!(1, facing);
    let (pos, facing) = cube_step_forward(&cube_map, (11, 7), 0, 1);
    assert_eq!((12, 8), pos);
    let (pos, facing) = cube_step_forward(&cube_map, (12, 8), 3, 1);
    assert_eq!((11, 7), pos);
    let (pos, facing) = cube_step_forward(&cube_map, (10,5), 0, 5);
    assert_eq!((14, 10), pos);
    assert_eq!(1, facing);
    
}

fn parse_path(input: &str) -> Vec<Move> {
    let mut path = Vec::<Move>::new();
    let mut current_number_str = String::new();
    for c in input.chars() {
        if c.is_numeric() {
            current_number_str.push(c);
        } else {
            let moves = current_number_str.parse::<i64>().unwrap();
            path.push(Move::Step(moves));
            current_number_str.clear();
            if c == 'R' {
                path.push(Move::Right);
            } else {
                assert_eq!(c, 'L');
                path.push(Move::Left);
            }
        }
    }
    if !current_number_str.is_empty() {
        let moves = current_number_str.parse::<i64>().unwrap();
        path.push(Move::Step(moves));
    }

    path
}

#[test]
fn test_parse_sample_path() {
    let path = parse_path(get_sample_path_input());
    assert_eq!(Move::Step(10), path[0]);
    assert_eq!(Move::Right, path[1]);
    assert_eq!(Move::Left, path[path.len() - 2]);
    assert_eq!(Move::Step(5), path[path.len() - 1]);
}

#[test]
fn test_parse_puzzle_path() {
    let path = parse_path(get_puzzle_path_input());
    assert_eq!(Move::Step(14), path[path.len() - 1]);
}

fn parse_map(input: &str) -> Vec<Vec<i8>> {
    // first find width
    let max_width = input
        .lines()
        .max_by_key(|line| line.chars().count())
        .unwrap()
        .len();

    let mut map: Vec<Vec<i8>> = vec![];
    for line in input.lines() {
        let mut row = vec![-1; max_width];
        for (x, c) in line.chars().enumerate() {
            match c {
                ' ' => {
                    row[x] = -1;
                }
                '.' => {
                    row[x] = 0;
                }
                '#' => {
                    row[x] = 1;
                }
                _ => {
                    panic!();
                }
            }
        }
        map.push(row);
    }

    map
}

fn cubify_map(
    map: &Map,
    face_size: usize,
    face_corners: Vec<((usize, usize), [(usize, usize); 4])>,
) -> CubeMap {
    let mut faces: Vec<Face> = vec![];
    for corner in face_corners {
        faces.push(Face {
            top_left: corner.0,
            bottom_right: v2add_usize(&corner.0, &(face_size, face_size)),
            connections: corner.1,
        });
    }

    // validate double links
    for (i, face) in faces.iter().enumerate() {
        for (j, connection) in face.connections.iter().enumerate() {
            if faces[connection.0].connections[connection.1] != (i, j) {
                println!("Connection from face {}, direction {} not matched by {}, {}- looking for {}, {}", connection.0, connection.1, i, j, faces[connection.0].connections[connection.1].0, faces[connection.0].connections[connection.1].1 );
            }
        }
    }

    (map.clone(), faces)
}

#[test]
fn test_parse_map() {
    let map = parse_map(get_sample_map_input());
    assert_eq!(12, map.len());
    assert_eq!(16, map[0].len());
    assert_eq!(-1, map[0][0]);
    assert_eq!(-1, map[0][12]);
    assert_eq!(-1, map[0][15]);
    assert_eq!(0, map[11][15]);
    assert_eq!(1, map[4][3]);
}

fn get_sample_map_input() -> &'static str {
    "        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#."
}

fn get_sample_path_input() -> &'static str {
    "10R5L5R10L4R5L5"
}

fn get_puzzle_map_input() -> &'static str {
    "                                                  ..................#..#.....#....................#............#.......#..##..........................
                                                  .#...#.....#.........................................#...##...#...#....#...................#........
                                                  ...........##.......#.........#.#.#...#..#.............#.............#........#.................##..
                                                  ..#........#.......#.............................................#......#...#.......................
                                                  ..............................#..........#...................#..................#.............#.....
                                                  .......#..#.....#.......#...#.........................#.........#...#..#...............#........##..
                                                  .........#.....#...............#...##.........#...................#..#..##.....#...........#.....#..
                                                  ..#.#.......#............#.#.............#...................#......................##..............
                                                  #......#......#......................#.......#.........#..............#.....#.#....#...#..#.........
                                                  #.#.......#.............................#.........................#.........#.#..#............#.....
                                                  #.................#...........#...#...#.....................................#............##...#.#...
                                                  ............#.....##............................##............#..#...#.......#..#..............#....
                                                  ....##..............#.................#...............#....#..#...#..............#.........#........
                                                  ......#..#...................................#.#......#..#....#............#..#......#..............
                                                  .........#........................................#........#....##....................#...........#.
                                                  .............#....##.#..#...#...##.#..................#.........#.##................................
                                                  ...............#...........#.........##................#............................................
                                                  #.###.#..#.........#......#........#......#......#.......................#.....................#....
                                                  ......................#................#.........#....#..##.....##....#....#........................
                                                  ...#.#.............................#.....##...#.#......#.......#.#..........#.###................#..
                                                  ......#....#.............#...............#.........#................#..#...........#......##......#.
                                                  .#.....................#...........#..............#...........................................##....
                                                  ....#...................................#...........................#..............#...........#....
                                                  ......................................#..........................#.............##...............#...
                                                  ###.......#.....#..#...........#..#.#.....#......#..........#........#.....#..#......#..............
                                                  .................#....#.............................#..................#.##.....#...................
                                                  ...............#...........................#....#......#..#...........#.............................
                                                  .........#....#........#.....................#.......#.#.#................#.........................
                                                  .........##..#....................................#.........#...#...#.....#.#......#................
                                                  ...##.......................#...........#...#........#...#.###....................#..........#......
                                                  #....................#......#...#....................#................#.#........##.........#......#
                                                  .....................................#................#..........#......#.........#...#.............
                                                  .....................#.....#..........................................#............#................
                                                  ....#..#..#........#..........#....................#....#...#..#..............##................#...
                                                  ............#.#...........##.............................#.............#..#.................#.......
                                                  .....#........#...##...............................#............#...#....#...#............##........
                                                  .....#...#.......#....#....#....#...................#...........#................#.........#........
                                                  .#.#.......#.#..........#..................#.........#....#...............................#.........
                                                  .........#..#.............#......#......#......#........#........#.#.#.................#....##......
                                                  .........#....#......................#.#.............###...#...#........#.......#....#..##..........
                                                  ...................##......#.........#...#...................#..................#...................
                                                  ..........#..........#....#.........#...................#............#..#...........................
                                                  ..........................#...............#....#...........#.................#.........#.#...#.#....
                                                  ......#....#...................#........................#........#......#...#......#................
                                                  #..................................#....#................#.#..............#.........................
                                                  .....#...#......................................#.....................#.....#......#........#.....#.
                                                  ..#.....#.......#..#...#...............................#.......#..................#......#.#........
                                                  ....................#.......#........#.........................##..#.........#.#.#........#....#....
                                                  ........#............#.......................................#........##............................
                                                  ....#..............#...........#....##............#.............#.....................#.............
                                                  .#........#.........#.....#.#...#..............#.#
                                                  ..#......#....................#.##........#...#...
                                                  ............#.......##......##........#...........
                                                  ......#.#.........#.......................#.......
                                                  ............#...................................#.
                                                  ............................#.#..............#....
                                                  .........................................#........
                                                  ............#.......#....#..#................#....
                                                  ...........#.#............#.....................#.
                                                  ..........#...#.............................#.....
                                                  ....................................#.#..##.......
                                                  ..#....#............#.....#.......................
                                                  .....#...#...........#............................
                                                  ...............#.......#..........#.....#.........
                                                  ..................................................
                                                  .....#.##.......#.##.....#.#.#...#................
                                                  ...........#..##.....#..................#.........
                                                  ....................#.......................#.....
                                                  .......#...##...............................#...##
                                                  ......#..............#........#.........#....#....
                                                  #.#.##...........#.......#..#........#...........#
                                                  ............##.................#.##...........#...
                                                  ...#.......#................#.#..#.#..............
                                                  ..#...........#............#..#.#......#.........#
                                                  ..................................................
                                                  ...........#..............#..............#........
                                                  ....#.#..............##.......#.#......#...#.#....
                                                  .........##.............#...#...#.................
                                                  ...#......#.....#...#..#..##..........#...........
                                                  .........#............#...........................
                                                  ......................#....#.........#............
                                                  .......................#...............#.......#..
                                                  ..........#.#....#............##................##
                                                  ..#......#..........#..#......#.......#.........#.
                                                  ................#.#...#.......#........#..........
                                                  ....#.....#..............................#........
                                                  ..............#............#...................#..
                                                  ....#.......#..#.....#...........#.....#......#...
                                                  ......#.....................#......#.#............
                                                  .....#.#....................#..........##.....#...
                                                  #...........#........#............................
                                                  ..#..........#..#...........#....................#
                                                  ...#.#.........#.........#.....#............#.....
                                                  ...#.........................#....#.#......#......
                                                  ....#.....#..............#.............#..........
                                                  ...#........#...........##.......................#
                                                  #....#.......#......#...#..........#..........#...
                                                  ........#...#.......#.....#...#................#..
                                                  #..............#.#....#..#...................#....
                                                  .........#.....#.#................................
#....##................#..........#...#.#......#...........................#.........#...........#..
#..........................#..........................#......#.......##.#...........................
#.......##...#.......#........#.....#......#...#.#....#.....#.......#.............#.................
.#......##.....#.#.........#........#...#.......................................#...........#.....#.
........#..###............#......##..#............#.......#.....#....#.......#......................
....#.#.....#...#....#.....#...#..#.#...........#..............#..................#............#....
..................#........#..#.....#..#...#....##..............#..##...#.......#.....#.............
......#...#.##......#......#.....#....#..........#.....#.....#...............................##.....
.......................#.....##.......#....#....................................#.................#.
.........#........................#..........#...#......#.....##....................................
.......#........#...................#.......#...#.............#.#..........#.#...#..#.....#.....#...
.....#..............#................#.....#........................#....#....#.....................
##...#.#..#..........#.................#....##......#.......#..................#.......#..#..#......
#............#...#.................#.................#.#..#.......#.#...............#...............
.#...........#...#.........................#........#.......##..................#....#..............
#......#.........#...................#........#........#.........#.............#.........#..........
.................................#...#......................#........#...#....#.....................
#..........#...........................#.............#....##.............###.........#........#.....
..............#..##...#..............#.......#............#...#.....#............#....#......#.#....
.........#.....#......#.....................................#.............#.......##..#.......#.....
..#..#.#......................#....#..........#...........#............................#....#...#...
#............#..............#...#...##.#.##........#..#....#....#...#...............#.#.............
..#...............#....#...#.........#..#......#..............#........#.........#..................
...........#....................#.....#...#...#..........#..........................................
......................#.............#....#..............#.......#....#.#...........................#
.................#.......#...#.......#...................#.#.......#.....#..........................
#............................#..#..........#..............##..#.............#..........#............
..................#.....................................#..#.#................................#....#
.#......#......#.......#........#.............#..#....................................#.....#..#....
........#..#.......................................#............#.....##.........#...........#..#...
......#..............#..#..#..#.............#..................#...#...........................#....
......................#...#..#.......#.#....................#...........#.......................#...
..#...........#..................#..#....#............................#....#...#.............##..#..
......#....#....#.#.......#...#...............#..........#..........#....#...#....................#.
......................................#................................#...#...........#....#..#....
................#.......................#.......................##.#........#...#............#......
......#..#.................#..........#......................#............#....#..#.......#.........
..##..............................#..#....#.................#........#.#.......#....................
...##.#....#..#..............#......#.......#..#.#.#.......#.#.......##..........#.....#..........##
......#..#....#....#..#.......#............#........###..........#.#..#...........#..........#..#..#
...#........#..........#....................#.........#.....................#...................#...
.#.#........#.................................#..............................#......#...............
.........#...........##........#..............##........#..........#.................#..............
......###....##...#.......................#....................#...................##.#..........#..
..............#....#........................................#.#.............#.....#.#....##.......#.
.........................................................................#.#.##.#..#.....#..........
...#.........#.........#......#..................#....................#..........................#..
..#...#.....#.............#..##..............................#.........##.#..#......................
.....................#...........#.....#......................#..............##..........#.......##.
.............#...............##.................#...#.........##........##.........................#
...#..............................................
.#.#..#...........#.............#.#......##......#
.................................#........##..#.#.
.......................#......#.........#..#......
..#.............#.#................#..............
.........##.##....................................
......#..##.............#....#.........#..........
...............#...........#..............##......
.........#.......#.#..#................#.....#..#.
.#..#.......................#.....#.........#..#..
.................#..#...................##.#..#...
........#...............#...##.............#..##..
...............#..........#.......................
..........#......#..#...................#.......#.
.#.......#..#..........................#..#.......
#.......#............................#............
.....#.#........#.........###............#........
.#.............#............##..#...#...#.#.......
.........#.......#..........#...#........#.#......
...#...........#...............#..................
........#................................#........
............#................................#....
....#........................#..##......#...#.....
.#.##..............#.#..................#.#...#.#.
.............#..........#...................#.....
..##....#....#..#...........#....#.##.............
.#.........#....................#...............#.
........#.#..........#...............#...#...#...#
...............#.#..................#.....##......
.....#....#.##..........#.......#....#...#....#.#.
..#.............#......#............#......#..#...
..#..#............#.#................#..#.........
.........#.......#......#......................#..
..........#.........................#.............
.##.......#...................#.........#..#......
...........#.....##.....#.#...#..........###......
...........#.......#..............................
.........#.......#...#.....#..........#....#..#...
......###.#....#.#..........##....#.......##.#....
.......#.............#..................#.....#...
.............#..........#......#......#...........
...#...........###.....#..#.................#.#...
..................#........##....#......#..#......
........................#.........##.....##.......
........#..#.#....#..........#...#...........#....
.....#........##........#............#..#.........
....................#............#.......#........
.................#.#...#....#.........#..##.......
...........#..........##....#..........#.....#....
..............#..........#.............#.........."
}

fn get_puzzle_path_input() -> &'static str {
    ""
}
